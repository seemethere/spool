...
Spool should become useful enough to build Spool with Spool as quickly as possible.
...
Early dogfooding task chain: simplify the **Worker Agent Contribution Surface** by extracting runner-side execution and delivery behavior into `spool-runner`, following `docs/RUNNER_EXTRACTION_PLAN.md`. This should be managed as dependency-chained Spool Tasks after the minimum dogfooding cutover point, not as a prerequisite for cutover.
...
- Implement the first Delegation Session contract in `docs/DELEGATION_SESSION.md`, centered on extension-native `spool_create_delegated_root_task` dogfood intake, with `spool delegate`, `spool delegate --refine <task_identifier>`, deterministic creation/refinement helpers, and the Pi-backed Delegation Interview as wrapper/fallback polish
- Review sessions
- Metrics export
- Richer transcript export/pruning policies
- Richer Spool Pi Extension tool set
