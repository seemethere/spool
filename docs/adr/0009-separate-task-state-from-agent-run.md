...
Spool will treat In Progress as a task lifecycle state, not as evidence that an agent process is currently running. Live execution attempts are modeled separately as Agent Runs, which keeps retries, restarts, and delegated blocking work from corrupting the meaning of the Task State.
