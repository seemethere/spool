...
Tasker will reject free-form state mutation for normal clients and instead enforce an explicit v1 transition model, with an operator-only repair override for exceptional fixes. This protects review, validation, and integration gates from accidental agent or script mistakes, while preserving a recovery path when operational data needs to be repaired.
