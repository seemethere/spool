...
Spool will use Integrating, not Merging, as the Task State for work being delivered by a queue's configured Delivery Backend. In v1, Local Worktree Delivery implements this with a local merge into the Main Branch, but the lifecycle term stays independent of GitHub, pull requests, or any future delivery backend.
