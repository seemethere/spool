...
Tasker's own dogfood Task Queue follows this local-first default. Ordinary Tasker development Tasks should not enter Human Review merely because extra confidence is useful; Worker Agents should use the advisory Subagent Review Loop before committing or requesting Integrating when risk warrants it. That advisory loop is outside Tasker's Review Agent domain model and does not record Review Decisions.
