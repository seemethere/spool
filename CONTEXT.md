...
- Dependency-aware multi-file Task creation extends **File-backed Task Creation** input handling by reading multiple file definitions, validating the whole same-queue dependency graph before mutation, and recording explicit blocking **Task** relationships; it is not a separate workflow concept. Same-batch references use front matter `batch_key` values and `blocking_task_keys`; existing **Blocking Tasks** still use `blocking_task_identifiers`.
