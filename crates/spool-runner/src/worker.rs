...
pub async fn run_worker_once(
    ...
    let data_dir =
        absolute_path(&request.data_dir).context("failed to resolve Spool data directory")?;
    ...
    let prepared_worktree =
        match prepare_local_worktree(pool, &claimed.task, &actor, &data_dir).await {
            ...
            Err(error) => {
                ...
                let finished = spool_db::finish_run(
                    pool,
                    &claimed.run.id,
    ...
    spool_db::heartbeat_run(pool, &claimed.run.id, request.lease_seconds, &actor).await?;
...
struct LauncherContext<'a> {
    ...
    claimed: &'a spool_db::ClaimedRun,
    actor: &'a spool_db::Actor,
...
struct LauncherResult {
    ...
    session_data: spool_db::UpsertLauncherSessionData,
}
...
async fn run_fake_launcher(
    ...
    spool_db::update_workpad_note(pool, &claimed.task.task.identifier, &fake_note, actor).await?;
...
async fn run_pi_launcher(
    ...
    let mut child = match command
        ...
        .env("SPOOL_ACTOR_KIND", "worker_agent")
        .env("SPOOL_ACTOR_ID", &actor.id)
        .env("SPOOL_ACTOR_DISPLAY_NAME", &actor.display_name)
        .env("SPOOL_AGENT_RUN_ID", &claimed.run.id)
        .env(
            "CARGO_TARGET_DIR",
            &prepared_worktree.shared_cargo_target_dir,
        )
...
fn build_worker_prompt(
    ...
    Ok(format!(
        "{base}\n\nTask Identifier: {}\nTask Title: {}\nTask State: {}\nAgent Run ID: {}\nLocal Worktree: {}\nShared Cargo Target Directory: {}\nCargo commands inherit CARGO_TARGET_DIR so Rust build artifacts are shared across Worker Agent Local Worktrees for this Managed Source Repository. This Spool-managed directory is safe to delete when reclaiming space.\n\n{WORKER_CONTEXT_BUNDLE_GUIDANCE}\n\nBefore requesting Integrating, commit intended changes as Task Commits on the Task Branch and verify the Local Worktree is clean with git status.\nUse Spool Pi Extension tools for Spool mutations. When finished, update criteria/validation/workpad and request the appropriate Task State Transition.\n",
...
pub async fn preflight_worker_claim(
    ...
    spool_db::check_migration_compatibility(pool)
        .await
...
fn preflight_local_worktree_delivery(queue: &spool_db::TaskQueue) -> Result<()> {
...
fn ensure_worktree_root_accessible(path: &Path) -> Result<()> {
    ...
    fs::write(&probe, b"spool preflight\n").with_context(|| {
        format!(
...
async fn prepare_local_worktree(
    ...
    let queue = spool_db::get_task_queue(pool, &task.task.task_queue_key)
        .await?
...
async fn upsert_task_link_with_lock_retry(
    pool: &SqlitePool,
    identifier: &str,
    input: spool_db::UpsertTaskLink,
    actor: &spool_db::Actor,
) -> Result<spool_db::TaskDetail> {
    const MAX_ATTEMPTS: usize = 5;
    let mut delay = Duration::from_millis(50);
    for attempt in 1..=MAX_ATTEMPTS {
        match spool_db::upsert_task_link(pool, identifier, &input, actor).await {
...
async fn prepare_fake_integrating_task(
    ...
    match request.fake_outcome.as_str() {
        ...
        "integrating-operational-failure" => {
            let queue = spool_db::get_task_queue(pool, &claimed.task.task.task_queue_key)
                .await?
                .with_context(|| {
...
mod tests {
    ...
    async fn worker_prompt_requires_context_bundle_context_plan_and_safe_fallback() {
        ...
        assert!(prompt.contains("Structured Spool fields remain authoritative"));
        ...
        assert!(prompt.contains("Preserve canonical project context reads"));
        assert!(prompt.contains("`CONTEXT.md`"));
        assert!(prompt.contains("`ROADMAP.md`"));
        assert!(prompt.contains("relevant ADRs or docs"));
    ...
    async fn fake_worker_prepares_local_worktree_and_records_task_links() {
        ...
        assert!(detail
            .task_links
            .iter()
            .any(|link| link.kind == "task_branch" && link.target == "spool/TASK-1"));
        ...
        let run_detail = spool_db::get_agent_run_detail(&pool, &run_id)
            .await
    ...
    async fn fake_worker_auto_integrates_successful_agent_gated_task() {
        ...
        assert!(log.contains(&format!("Spool-Agent-Run: {run_id}")));
    }
    ...
    async fn local_integration_retry_succeeds_after_managed_source_becomes_clean() {
        ...
        let actor = spool_db::Actor::operator("retry-adapter");
    ...
    async fn pi_worker_fails_zero_exit_without_agent_end_event() {
        ...
        let run = spool_db::get_agent_run(&pool, &run_id)
            .await
            .expect("load run")
            .expect("run");
        assert_eq!(
    ...
    fn local_worktree_setup_rejects_existing_plain_directory() {
        ...
        let error = setup_local_worktree(&repo, "main", "spool/TASK-1", &worktree)
    ...
    fn local_worktree_setup_rejects_unrelated_existing_worktree_path() {
        ...
        git(&unrelated, &["checkout", "-b", "spool/TASK-1"]);

        let error = setup_local_worktree(&repo, "main", "spool/TASK-1", &unrelated)
