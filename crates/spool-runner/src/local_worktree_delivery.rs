...
const VALIDATION_COMMANDS_FILE: &str = ".spool/validation-commands.txt";
...
fn required_task_link(detail: &spool_db::TaskDetail, kind: &str) -> Result<String> {
    detail
        .task_links
        .iter()
...
struct LocalWorktreeIntegrationAdapter<'a> {
    pool: &'a SqlitePool,
    task: &'a spool_db::TaskDetail,
    queue: &'a spool_db::TaskQueue,
...
impl<'a> LocalWorktreeIntegrationAdapter<'a> {
    async fn integrate(&mut self) -> Result<LocalIntegrationResult> {
        ...
        .with_context(|| {
            format!(
                "Final Commit {final_commit} was created but Spool could not record the successful Integration Outcome; operator repair required"
            )
        })?;
        self.transition("done").await.with_context(|| {
            format!(
                "Final Commit {final_commit} was created but Spool could not move the Task to Done; operator repair required"
            )
        })?;
        let mut summary = format!(
    ...
    async fn auto_refresh_stale_validated_base(
        ...
        spool_db::record_task_validated_base_commit(
            self.pool,
            &self.task.task.identifier,
            &stale.current_main,
            self.actor,
...
mod integration_tests {
    ...
    async fn local_worktree_integration_squash_merges_and_cleans_successful_task() {
        ...
        assert_eq!(
            ...
            commit_metadata::SpoolCommitTrailers {
        ...
        assert!(parsed_by_git.contains("Spool-Task: TASK-1"));
        assert!(parsed_by_git.contains("Spool-Queue: TASK"));
        assert!(!worktree.exists());
        assert!(!Command::new("git")
            .arg("-C")
            .arg(&repo)
            .args(["show-ref", "--verify", "--quiet", "refs/heads/spool/TASK-1"])
            .status()
    ...
    async fn local_worktree_integration_stale_validated_base_auto_refresh_succeeds_without_agent_run(
        ...
        fs::create_dir_all(repo.join(".spool")).expect("spool dir");
        fs::write(
            repo.join(".spool/validation-commands.txt"),
            "test -f feature.txt\n",
        )
        ...
        git(
            &repo,
            &["add", ".spool/validation-commands.txt", "main-only.txt"],
        );
        git(&repo, &["commit", "-m", "move main"]);
        let current_main = git_output(&repo, &["rev-parse", "main"], "read main head")
            .expect("main head")
    ...
    async fn local_worktree_integration_stale_validated_base_conflict_moves_to_rework() {
        ...
        fs::create_dir_all(repo.join(".spool")).expect("spool dir");
        fs::write(repo.join(".spool/validation-commands.txt"), "true\n").expect("commands");
        fs::write(repo.join("feature.txt"), "main feature\n").expect("conflict");
        git(
            &repo,
            &["add", ".spool/validation-commands.txt", "feature.txt"],
        );
    ...
    async fn local_worktree_integration_stale_validated_base_validation_failure_moves_to_rework() {
        ...
        record_current_main_as_stale_base(&pool, &repo).await;
        fs::create_dir_all(repo.join(".spool")).expect("spool dir");
        fs::write(repo.join(".spool/validation-commands.txt"), "false\n").expect("commands");
        fs::write(repo.join("main-only.txt"), "main moved\n").expect("main change");
        git(
            &repo,
            &["add", ".spool/validation-commands.txt", "main-only.txt"],
    ...
    fn init_git_repo(repo: &Path) {
        ...
        run_git(
            repo,
            &["config", "user.email", "spool@example.test"],
            "config email",
        )
        .expect("email");
        run_git(repo, &["config", "user.name", "Spool Test"], "config name").expect("name");
