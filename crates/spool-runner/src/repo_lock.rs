...
pub struct RepoOperationLockFile {
    pub spool_repo_operation_lock: bool,
    pub queue: String,
    pub pid: u32,
...
pub fn blocked_message(active: &ActiveRepoOperationLock) -> String {
    format!(
        "Managed Source Repository operation lock is held for Task Queue {} by pid {} operation={}{} at {}. Workers must not claim Tasks while the Managed Source Repository is being mutated. Wait for the operation to finish or use `spool merge lock release --queue {}` only after explicit operator verification.",
        active.lock.queue,
        active.lock.pid,
...
fn lock_contents(
    ...
    serde_json::to_string_pretty(&RepoOperationLockFile {
        spool_repo_operation_lock: true,
