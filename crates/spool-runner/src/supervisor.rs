    ...
    while Instant::now() < deadline {
        ...
        while index < active.len() {
            if active[index].child.try_wait()?.is_some() {
                ...
                for run in stuck {
                    println!(
                        "stuck Agent Run {} for Task {} remains active after worker {} exited; suggested recovery: spool run fail {} --reason <reason>",
                        run.agent_run_id,
...
fn spawn_worker(command: &[String], actor: &str, status_path: PathBuf) -> Result<WorkerProcess> {
    ...
    cmd.args(&command[1..])
        .arg("--actor")
        .arg(actor)
        .env("SPOOL_WORKER_STATUS_PATH", &status_path)
        .stdout(Stdio::piped())
        .stderr(Stdio::piped());
...
struct WorkerStatusReport {
    spool_worker_status: Option<bool>,
    ...
    fn refresh(
        ...
        for file in &self.files {
            ...
            for line in text.lines() {
                ...
                if report.spool_worker_status != Some(true) {
                    continue;
                }
...
fn active_supervisor_lock_message(queue: &str, path: &Path) -> String {
    format!(
        "another active supervisor appears to hold Task Queue {queue}; refusing to start overlapping Worker Loop claims. Lock file: {}. Inspect the existing supervisor process and Spool state with `ps -p <pid>` and `spool status --queue {queue}`. If the supervisor crashed and the lock is stale, rerun after the process exits; Spool removes stale locks automatically when the recorded pid is gone. To clear corrupted stale state, delete the lock file manually. Use --allow-overlap only for intentional recovery.",
        path.display()
    )
}
...
fn supervisor_status_dir(run_prefix: &str) -> Result<PathBuf> {
    let dir = std::env::temp_dir().join(format!("spool-{run_prefix}"));
...
async fn retry_due_integrations(
    ...
    let retries = spool_db::due_integration_retries(pool, queue).await?;
    if retries.is_empty() {
        return Ok(());
    }
    let actor = spool_db::Actor::operator(format!("{run_prefix}-integration-retry"));
...
async fn print_progress(pool: &SqlitePool, queue: &str) -> Result<()> {
    let active_runs = spool_db::active_agent_runs_for_status(pool).await?;
    let holds = spool_db::active_retry_holds_for_status(pool).await?;
...
async fn unblocking_state(
    ...
    let active_runs = spool_db::active_agent_runs_for_status(pool)
        .await?
        .into_iter()
...
async fn active_runs_for_worker(
    ...
    queue: &str,
    worker_id: &str,
) -> Result<Vec<spool_db::ActiveAgentRunStatus>> {
    Ok(spool_db::active_agent_runs_for_status(pool)
        .await?
        .into_iter()
...
fn active_run_progress_message(run: &spool_db::ActiveAgentRunStatus) -> String {
    format!(
...
"#,
    ...
    async fn supervisor_stops_when_worker_reports_completed_handoff() {
        ...
        fs::write(
            &script,
            "#!/bin/sh\necho '{\"spool_worker_status\":true,\"task_identifier\":\"TASK-1\",\"agent_run_id\":\"run-1\",\"status\":\"completion_intent\",\"message\":\"handoff\"}' >> \"$SPOOL_WORKER_STATUS_PATH\"\nexit 0\n",
        )
        .expect("write script");
        make_executable(&script);
        let pool = empty_pool(temp.path()).await;
    ...
    fn supervisor_lock_refuses_same_queue_overlap() {
        ...
        assert!(message.contains("spool status --queue TASK"));
    ...
    fn supervisor_active_run_progress_message_stays_compact_without_task_title() {
        let message = active_run_progress_message(&spool_db::ActiveAgentRunStatus {
    ...
    fn supervisor_run_prefixes_are_unique_and_worker_ids_use_prefix() {
        let first = supervisor_run_prefix(&SupervisorOptions {
            ...
            lock_dir: PathBuf::from("/tmp/spool-supervisor-locks"),
            data_dir: PathBuf::from("/tmp/spool-data"),
            allow_overlap: false,
            watch: false,
            run_prefix: None,
        });
    ...
    async fn empty_pool(path: &std::path::Path) -> SqlitePool {
        let db_path = path.join("spool.db");
    ...
    async fn seed_task(pool: &SqlitePool, state: &str) {
        ...
        init_git_repo(&repo);
        sqlx::query(
            "INSERT INTO task_queues (id, key, name, delivery_backend, managed_source_repository, main_branch, worktree_root, branch_template, done_worktree_retention) VALUES ('queue-1', 'TASK', 'Spool', 'local_worktree', ?, 'main', ?, 'spool/{identifier}', false)",
        )
