...
pub(crate) async fn init(paths: &TaskerPaths, db_path_overridden: bool) -> Result<()> {
...
pub(crate) fn manual_migration_command(paths: &TaskerPaths, db_path_overridden: bool) -> String {
    let mut parts = vec![
        "tasker".to_string(),
        "--config".to_string(),
