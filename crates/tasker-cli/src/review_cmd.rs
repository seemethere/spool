...
pub(crate) async fn review(
    ...
    let bundle = tasker_db::get_task_context_bundle(&pool, &options.identifier)
        .await?
        .with_context(|| format!("Task {} not found", options.identifier))?;
    if bundle.task.task.state != "human_review" {
        anyhow::bail!(
            "tasker review requires Task {} to be in Human Review; current Task State is {}",
            options.identifier,
    ...
    let outcome =
        tasker_runner::review::run_review_session(tasker_runner::review::ReviewSessionRequest {
