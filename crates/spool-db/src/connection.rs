...
pub async fn check_migration_compatibility(pool: &SqlitePool) -> Result<()> {
    ...
    if !pending.is_empty() {
        anyhow::bail!(
            "Spool database has pending SQLite migrations {:?}. Normal commands validate schema compatibility but do not apply migrations. Run `spool db migrate` from the trusted Managed Source Repository Main Branch to upgrade the Task Backend.",
            pending
        );
    }
...
pub async fn pending_migration_versions(pool: &SqlitePool) -> Result<Vec<i64>> {
    ...
    if migrations_table_exists == 0 {
        anyhow::bail!(
            "Spool database schema is not initialized. Run `spool init` for a new Task Backend, or run `spool db migrate` from the Managed Source Repository Main Branch for an existing Task Backend."
        );
    }
    ...
    {
        anyhow::bail!(
            "Spool database migration {version} is marked failed/dirty. Restore from backup or repair the migration state before running Spool commands."
        );
    }
