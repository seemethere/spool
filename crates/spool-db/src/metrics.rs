        ...
        if canonical == "bash" {
            if let Some(command) = tool_string_arg(value, &["command", "cmd"]) {
                ...
                *self
                    ...
                    .or_insert(0) += 1;
                if let Some(signature) = spool_context_fetch_signature(&command) {
                    let count = self
                        .spool_context_fetch_signatures
                        .entry(signature)
                        .or_insert(0);
                    *count += 1;
                    if *count > 1 {
            ...
            return false;
        } else if is_spool_context_tool(&canonical) {
            let count = self
                .spool_context_fetch_signatures
...
fn shell_command_category(command: &str) -> &'static str {
    ...
    if tokens.iter().any(|token| token == "spool-local")
        || is_direct_spool_invocation(&lowered)
...
fn is_direct_spool_invocation(command: &str) -> bool {
    let trimmed = command.trim_start_matches(|ch: char| ch.is_ascii_whitespace() || ch == '(');
    trimmed.starts_with("spool ")
        || trimmed.contains("&& spool ")
        || trimmed.contains("; spool ")
        || trimmed.contains("| spool ")
}
...
fn spool_context_fetch_signature(command: &str) -> Option<String> {
    let lowered = command.to_ascii_lowercase();
    let normalized = lowered.split_whitespace().collect::<Vec<_>>().join(" ");
    let context_kind = if normalized.contains("spool-local task show")
        || normalized.contains("spool task show")
        || normalized.contains("task show")
    {
        "task_show"
    } else if normalized.contains("spool-local queue show")
        || normalized.contains("spool queue show")
        || normalized.contains("queue show")
    {
        "queue_show"
    } else if normalized.contains("spool-local run show")
        || normalized.contains("spool run show")
        || normalized.contains("run show")
    {
        "run_show"
    } else if normalized.contains("spool-local status")
        || normalized.contains("spool status")
        || normalized == "status"
    {
        "status"
    } else {
...
fn is_spool_context_tool(canonical: &str) -> bool {
...
mod shell_command_category_tests {
    ...
    fn classifies_common_dogfood_shell_commands_without_raw_command_storage() {
        let cases = [
            ...
            (
                "sqlite3 .spool/data/spool.db 'select count(*) from tasks'",
                "database",
            ),
            ("ps aux | grep spool", "process_supervisor"),
            ("pgrep -fl spool", "process_supervisor"),
