...
const DEFAULT_REVIEW_ROLE_PROMPT: &str = "You are a Tasker Review Agent running in a human-present Review Session. You are not a Worker Agent and you are not a pre-dogfooding advisory subagent reviewer. Prepare or summarize the Review Packet, guide the human to one explicit approve or rework Review Decision, and record that Review Decision through the deterministic Tasker review-decision path.";
...
pub async fn run_review_session(request: ReviewSessionRequest) -> Result<ReviewSessionOutcome> {
    ...
    let mut child = command
        ...
        .env("TASKER_ACTOR_KIND", "review_agent")
...
fn build_review_prompt(
    ...
    let override_path = managed_source_repository.join(".tasker/prompts/review.md");
...
mod tests {
    ...
    fn review_prompt_uses_builtin_prompt_and_review_packet() {
        ...
        assert!(prompt.contains("Tasker Review Agent"));
...
"#,
        ...
        assert!(captured.contains("review_agent:reviewer:http://tasker.test"));
    }
}
