...
use spool_runner::local_worktree_delivery;
...
pub(crate) async fn merge(
    ...
    match command {
        MergeCommand::Queue { queue } => {
            let rows = spool_db::merge_queue_tasks(&pool, queue.as_deref()).await?;
        ...
        MergeCommand::Inspect { identifier } => {
            ...
            let queue = spool_db::get_task_queue(&pool, &detail.task.task_queue_key)
                .await?
                .with_context(|| format!("Task Queue {} not found", detail.task.task_queue_key))?;
            let latest_run =
                spool_db::get_latest_agent_run_detail_for_task(&pool, &identifier).await?;
        ...
        MergeCommand::Lock { command } => match command {
            MergeLockCommand::Acquire {
                ...
                println!(
                    "release after operator verification: spool merge lock release --queue {}",
                    active.lock.queue
                );
            }
        ...
        MergeCommand::Done {
            ...
            if !manual {
                anyhow::bail!(
                    "refusing to mark Task Done without --manual confirmation that the Local Merge was performed outside Spool"
                );
            }
            let current = spool_db::get_task_detail(&pool, &identifier)
                .await?
                .with_context(|| format!("Task {identifier} not found"))?;
...
pub(crate) async fn integrate_local_worktree(
    ...
    let latest_run = spool_db::get_latest_agent_run_detail_for_task(pool, identifier).await?;
...
async fn retry_local_worktree_integration(
    ...
    if !force {
        match latest_integration_outcome_for_task(pool, identifier).await? {
            ...
            None => anyhow::bail!(
                "refusing Integration retry for Task {identifier}: no previous Integration Outcome found; use `spool merge integrate` for a first integration attempt"
            ),
        }
    }
...
fn print_manual_merge_queue(rows: &[spool_db::MergeQueueTask]) {
    println!("Manual Dogfood Merge queue");
    println!("temporary helper: read-only view for Integrating Tasks; Git operations remain operator-side; Spool Service performs no Git mutations");
    ...
    for row in rows {
        ...
        println!("  Detail: spool merge inspect {}", row.task_identifier);
        println!();
    }
}
...
fn print_manual_merge_inspection(
    detail: &spool_db::TaskDetail,
    queue: &spool_db::TaskQueue,
    latest_run: Option<&spool_db::AgentRunDetail>,
    ...
    println!("temporary helper: Git operations remain operator-side; Spool Service performs no Git mutations");
    println!();
...
pub(crate) fn manual_squash_integration_guidance(
    ...
    vec![
        format!(
            "Before mutating Main Branch manually, run: spool merge lock acquire --queue {queue_key} --operation manual_integration --task {task_identifier}."
        ),
        "Inspect Spool state, latest Agent Run, Run Transcript, and Workpad Note.".to_string(),
        ...
        "Do not use Task Branch ancestry as completion proof after a squash integration; Spool DB state, Integration Outcomes, Audit Events, and the Final Commit are authoritative.".to_string(),
        "From the Managed Source Repository, run post-merge batch validation before marking the batch Done.".to_string(),
        format!("Release the operation lock: spool merge lock release --queue {queue_key}"),
        format!("After validation, run: spool merge done {task_identifier} --manual"),
    ]
}
