...
fn supervise_help_points_title_seekers_to_status_and_monitor() {
    ...
    assert!(help.contains("spool status"));
    assert!(help.contains("spool monitor"));
...
async fn open_pool_rejects_pending_migrations_before_work_can_claim() {
    ...
    let db_path = data_dir.join("spool.db");
    let paths = SpoolPaths {
        config_path,
        data_dir,
        db_path: db_path.clone(),
    };
    SpoolConfig::default_for_paths(&paths)
...
fn status_json_exposes_lifecycle_telemetry_shape() {
    let rows = vec![
        spool_db::QueueStatus {
            queue_key: "TASK".to_string(),
            queue_name: "Spool".to_string(),
    ...
    let active_runs = vec![spool_db::ActiveAgentRunStatus {
        queue_key: "TASK".to_string(),
    ...
    let active_holds = vec![spool_db::ActiveRetryHoldStatus {
        queue_key: "TASK".to_string(),
    ...
    let status_tasks = vec![spool_db::TaskStatusSummary {
        queue_key: "TASK".to_string(),
    ...
    let conflicts = vec![spool_db::TaskConflictGroup {
        queue_key: "TASK".to_string(),
        target: "crates/spool-cli".to_string(),
        task_count: 2,
        tasks: "TASK-3,TASK-4".to_string(),
    }];
    let integration_retries = vec![spool_db::IntegrationRetryStatus {
        queue_key: "TASK".to_string(),
...
fn project_config_guard_refuses_mutating_command_when_inactive() {
    ...
    let project_config = repo.join(".spool/config.toml");
    write_config(&project_config, &repo.join(".spool/data/spool.db"));
    ...
    assert!(message.contains("refusing mutating Spool command"));
    assert!(message.contains("--config .spool/config.toml"));
    assert!(message.contains("bin/spool-local"));
...
fn project_config_guard_allows_explicit_config_for_mutating_command() {
    ...
    fs::create_dir_all(&nested).expect("nested repo dir");
    let project_config = repo.join(".spool/config.toml");
    let project_db = repo.join(".spool/data/spool.db");
    write_config(&project_config, &project_db);
    let paths = SpoolPaths::resolve(
        temp.path().join("home"),
        PathOverrides {
...
async fn supervisor_migration_pause_reports_pending_without_claiming_tasks() {
    ...
    assert!(!spool_db::pending_migration_versions(&pool)
        .await
        .expect("pending")
        .is_empty());
...
async fn supervisor_auto_migrate_guard_refuses_untrusted_or_dirty_checkouts() {
    ...
    git(&repo, &["checkout", "-b", "spool/TASK-2"]);
...
fn supervise_default_worker_command_forwards_project_config_and_child_infers_project_data_dir() {
    let temp = tempfile::tempdir().expect("tempdir");
    let config_path = temp.path().join("repo/.spool/config.toml");
    let configured_db = temp.path().join("repo/.spool/data/project.db");
    ...
    assert_eq!(paths.data_dir, temp.path().join("repo/.spool/data"));
    assert_eq!(
...
fn supervise_default_worker_command_forwards_explicit_db_path() {
    let command = default_worker_command(
        Path::new("/tmp/spool"),
        &PathForwardingOptions {
            config: Some(PathBuf::from(".spool/config.toml")),
            data_dir: Some(PathBuf::from(".spool/data")),
            db_path: Some(PathBuf::from(".spool/data/spool.db")),
        },
        &SuperviseOptions {
            queue: "TASK".to_string(),
            concurrency: 1,
    ...
    assert!(command
        .windows(2)
        .any(|args| args == ["--config", ".spool/config.toml"]));
    assert!(command
        .windows(2)
        .any(|args| args == ["--data-dir", ".spool/data"]));
    assert!(command
        .windows(2)
        .any(|args| args == ["--db-path", ".spool/data/spool.db"]));
}
...
fn project_config_guard_allows_read_only_command_when_inactive() {
    ...
    fs::create_dir_all(&nested).expect("nested repo dir");
    write_config(
        &repo.join(".spool/config.toml"),
        &repo.join(".spool/data/spool.db"),
    );
    let paths = SpoolPaths::resolve(temp.path().join("home"), PathOverrides::default());
    let cli = Cli {
        config: None,
...
fn active_context_for_task_creation_renders_paths_and_queue_key() {
    ...
    let configured_db = temp.path().join("project/spool.db");
    ...
    let context = active_spool_context(&command, &paths, false).expect("active context");
    let rendered = render_active_spool_context(&context);
...
fn active_context_for_supervisor_renders_queue_key_and_db_override() {
    let temp = tempfile::tempdir().expect("tempdir");
    let override_db = temp.path().join("override/spool.db");
    let paths = SpoolPaths::resolve(
        temp.path(),
        PathOverrides {
    ...
    let context = active_spool_context(&command, &paths, true).expect("active context");
    let rendered = render_active_spool_context(&context);
...
fn write_config(config_path: &Path, db_path: &Path) {
    ...
    fs::write(
        ...
        format!(
            "[service]\nbind_addr = \"{}\"\n\n[database]\npath = \"{}\"\n",
            spool_config::DEFAULT_BIND_ADDR,
            db_path.display()
        ),
    )
    .expect("write config");
...
async fn init_creates_parent_directory_for_custom_db_path() {
    ...
    let paths = SpoolPaths::resolve(
        temp.path(),
        PathOverrides {
            db_path: Some(temp.path().join("custom/sub/spool.db")),
            ..PathOverrides::default()
        },
    );
...
async fn init_uses_existing_config_database_path() {
    let temp = tempfile::tempdir().expect("tempdir");
    let paths = SpoolPaths::resolve(temp.path(), PathOverrides::default());
    let configured_db_path = temp.path().join("configured/spool.db");
    let config = SpoolConfig {
        service: spool_config::ServiceConfig {
            bind_addr: spool_config::DEFAULT_BIND_ADDR.to_string(),
        },
        database: spool_config::DatabaseConfig {
            path: configured_db_path.clone(),
        },
        telemetry: spool_config::TelemetryConfig::default(),
    };
...
"#,
    ...
    .expect("fake work");
    let pool = open_pool(&paths, false).await.expect("pool");
    let missing_run = spool_db::get_agent_run(&pool, "not-a-real-run")
        .await
        .expect("get missing run");
    assert!(missing_run.is_none());
    let detail = spool_db::get_task_detail(&pool, "TASK-1")
        .await
        .expect("get task")
        .expect("task");
    assert!(detail
    ...
    let merge_queue = spool_db::merge_queue_tasks(&pool, Some("TASK"))
        .await
    ...
    assert!(
        spool_runner::repo_lock::active_lock(&paths.data_dir, "TASK")
            .expect("active lock")
            .is_some()
    );
    merge(
    ...
    .await
    .expect("retry task");
    let retry_detail = spool_db::get_task_detail(&pool, "TASK-2")
        .await
...
async fn task_review_decision_command_records_approve_to_integrating() {
    ...
    let detail = spool_db::get_task_detail(&pool, "TASK-1")
        ...
        .expect("task exists");
    assert_eq!(detail.task.state, "integrating");
    let events = spool_db::list_task_audit_events(&pool, "TASK-1")
        .await
        .expect("audit events");
...
async fn seed_cli_human_review_task(temp: &Path, title: &str) -> (SpoolPaths, sqlx::SqlitePool) {
    let paths = SpoolPaths::resolve(temp, PathOverrides::default());
    init(&paths, false).await.expect("init");
...
"#
    ...
    spool_db::update_acceptance_criterion_status(
        ...
        "TASK-1",
        1,
        &spool_db::UpdateRequirementStatus {
            status: "satisfied".to_string(),
            waiver_reason: None,
            validated_base_commit: None,
        },
...
async fn review_command_launches_pi_backed_interactive_review_session() {
    ...
    let repo = temp.path().join("repo");
    init_git_repo(&repo);
    let prompt_dir = repo.join(".spool/prompts");
...
"#,
    ...
    review(
        ...
        ReviewOptions {
            identifier: "TASK-1".to_string(),
            actor: "reviewer".to_string(),
            api_url: Some("http://spool.test".to_string()),
...
async fn worker_integrating_transition_rejects_dirty_local_worktree_without_state_change() {
    ...
    assert!(message.contains("spool/TASK-1"));
...
async fn seed_in_progress_local_task(
    ...
    .await
    .expect("task");
    git(&repo, &["branch", "spool/TASK-1", "main"]);
...
fn init_git_repo(repo: &Path) {
    ...
    git(repo, &["config", "user.email", "spool@example.test"]);
    git(repo, &["config", "user.name", "Spool Test"]);
...
fn merge_inspection_git_commands_report_cleanliness_and_main_diff() {
    ...
    let repo = temp.path().join("repo");
    init_git_repo(&repo);
    git(&repo, &["checkout", "-b", "spool/TASK-1"]);
...
async fn bootstrap_lint_succeeds_for_valid_task_file_without_database() {
    let temp = tempfile::tempdir().expect("tempdir");
    let paths = SpoolPaths::resolve(temp.path().join("missing-config"), PathOverrides::default());
    let task_file = temp.path().join("task.md");
    fs::write(
        &task_file,
        "---\ntitle: Test lint\npriority: high\nstate: backlog\nacceptance_criteria:\n  - It works\nvalidation_items:\n  - Tests pass\n---\nBrief\n",
