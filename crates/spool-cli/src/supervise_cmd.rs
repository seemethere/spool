...
pub(crate) async fn supervise(
    ...
    let command = if let Some(command) = options.worker_command {
        ...
        let exe = std::env::current_exe().context("failed to resolve current spool executable")?;
    ...
    let outcome = spool_runner::supervisor::supervise_batch(
        &pool,
        spool_runner::supervisor::SupervisorOptions {
            queue: options.queue,
