...
struct QueueTelemetry<'a> {
    ...
    active_runs: Vec<&'a spool_db::ActiveAgentRunStatus>,
    ready_task_summaries: Vec<&'a spool_db::TaskStatusSummary>,
    rework_task_summaries: Vec<&'a spool_db::TaskStatusSummary>,
    integrating_task_summaries: Vec<&'a spool_db::TaskStatusSummary>,
    advisory_conflict_hints: Vec<&'a spool_db::TaskConflictGroup>,
    retry_holds: Vec<&'a spool_db::ActiveRetryHoldStatus>,
    integration_retries: Vec<&'a spool_db::IntegrationRetryStatus>,
}
...
pub(crate) fn build_status_telemetry<'a>(
    rows: &'a [spool_db::QueueStatus],
    active_runs: &'a [spool_db::ActiveAgentRunStatus],
    active_holds: &'a [spool_db::ActiveRetryHoldStatus],
    status_tasks: &'a [spool_db::TaskStatusSummary],
    conflict_groups: &'a [spool_db::TaskConflictGroup],
    integration_retries: &'a [spool_db::IntegrationRetryStatus],
) -> StatusTelemetry<'a> {
...
pub(crate) async fn status(paths: &SpoolPaths, db_path_overridden: bool, json: bool) -> Result<()> {
    let pool = open_pool(paths, db_path_overridden).await?;
    let rows = spool_db::status_by_queue_and_state(&pool).await?;
    if rows.is_empty() {
        if json {
            println!("{{\n  \"queues\": []\n}}");
        } else {
    ...
    let active_holds = spool_db::active_retry_holds_for_status(&pool).await?;
    let status_tasks =
        spool_db::tasks_for_status_by_states(&pool, &["ready", "rework", "integrating"]).await?;
    let conflict_groups = spool_db::task_conflict_groups_for_status(&pool).await?;
    let integration_retries = spool_db::integration_retries_for_status(&pool).await?;
            ...
            if let Some(limit) = row.queue_concurrency_limit {
                if row.active_agent_runs >= limit {
                    ...
                    if active_integrating > 0 {
                        println!(
                            "  capacity saturated: active Agent Runs count against Queue Concurrency Limit, including {active_integrating} Integrating run(s). Unblock by waiting for completion or lease expiry, inspecting/finishing stuck runs with `spool run show`/`spool run fail`, or raising/clearing the Queue Concurrency Limit only if local resources permit."
                        );
                    } else {
                        println!(
                            "  capacity saturated: active Agent Runs count against Queue Concurrency Limit. Unblock by waiting for completion or lease expiry, inspecting/finishing stuck runs with `spool run show`/`spool run fail`, or raising/clearing the Queue Concurrency Limit only if local resources permit."
                        );
                    }
                }
            }
