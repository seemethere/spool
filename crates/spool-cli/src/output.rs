...
pub fn write_task_detail(mut writer: impl Write, detail: &spool_db::TaskDetail) -> Result<()> {
...
pub fn write_queue(mut writer: impl Write, queue: &spool_db::TaskQueue) -> Result<()> {
...
pub fn print_task_detail(detail: &spool_db::TaskDetail) -> Result<()> {
...
fn primary_task_link_target<'a>(links: &'a [spool_db::TaskLink], kind: &str) -> Option<&'a str> {
    links
        .iter()
...
struct RunTelemetry<'a> {
    ...
    normalized_metrics: Option<&'a spool_db::AgentRunMetrics>,
    efficiency_budget: Option<EfficiencyBudgetStatus>,
}

fn run_telemetry(detail: &spool_db::AgentRunDetail) -> RunTelemetry<'_> {
...
pub fn write_run_detail_json(
    mut writer: impl Write,
    detail: &spool_db::AgentRunDetail,
) -> Result<()> {
...
pub fn write_run_detail(mut writer: impl Write, detail: &spool_db::AgentRunDetail) -> Result<()> {
...
fn budget_status_for_metrics(metrics: &spool_db::AgentRunMetrics) -> EfficiencyBudgetStatus {
...
pub fn print_queue(queue: &spool_db::TaskQueue) -> Result<()> {
    write_queue(std::io::stdout(), queue)
}

pub fn print_run_detail(detail: &spool_db::AgentRunDetail) -> Result<()> {
...
mod tests {
    ...
    fn sample_run_detail(
        ...
        spool_db::AgentRunDetail {
            ...
            launcher_session_data: Some(spool_db::LauncherSessionData {
    ...
    fn task_detail_marks_conflict_hints_as_advisory_likely_paths() {
        let detail = spool_db::TaskDetail {
            task: spool_db::Task {
                id: "task-1".to_string(),
                task_queue_id: "queue-1".to_string(),
                task_queue_key: "TASK".to_string(),
            ...
            workpad_note: None,
            task_links: Vec::new(),
            conflict_hints: vec![spool_db::TaskConflictHint {
                id: "hint-1".to_string(),
                task_id: "task-1".to_string(),
                position: 1,
                target: "crates/spool-cli/src/output.rs".to_string(),
            }],
            conflict_overlaps: Vec::new(),
            blocking_tasks: Vec::new(),
            blocked_tasks: Vec::new(),
        ...
        assert!(text.contains("crates/spool-cli/src/output.rs"));
    }
    ...
    fn run_detail_includes_concise_efficiency_budget_status() {
        ...
        detail.metrics = Some(spool_db::AgentRunMetrics {
            agent_run_id: "run-1".to_string(),
            derivation_version: spool_db::CURRENT_AGENT_RUN_METRICS_DERIVATION_VERSION,
