...
pub async fn backfill_agent_run_metrics(
    ...
    let mut summary = BackfillSummary {
        ...
        current_derivation_version: spool_db::CURRENT_AGENT_RUN_METRICS_DERIVATION_VERSION,
    ...
    for row in rows {
        ...
        let Some(computed) = spool_db::compute_agent_run_metrics(pool, &row.agent_run_id).await?
        else {
            continue;
        };
        let existing = spool_db::get_agent_run_metrics(pool, &row.agent_run_id).await?;
        ...
        if changed {
            ...
            if options.write {
                spool_db::refresh_agent_run_metrics(pool, &row.agent_run_id).await?;
...
fn token_metric_kind(metrics: &spool_db::ComputedAgentRunMetrics) -> &'static str {
...
fn metrics_changed(
    existing: &spool_db::AgentRunMetrics,
    computed: &spool_db::ComputedAgentRunMetrics,
) -> bool {
...
pub fn render_summary(summary: &TelemetrySummary) -> String {
    ...
    writeln!(
        output,
        "efficiency metrics: {} run(s), {} tool call(s), {} tool error(s), {} repeated failed tool attempt(s), {} repeated read(s), {} repeated Spool context fetch(es), assistant/user turns {}/{}, max tokens input/output/total {}/{}/{}, cache read/write {}/{}, max context tokens {}",
...
fn write_trend_bucket(output: &mut String, label: &str, bucket: &TrendBucket) {
    ...
    writeln!(
        output,
        "    tool calls/errors: {}/{}; repeated reads={} repeated Spool context fetches={}",
        bucket.tool_calls,
        bucket.tool_errors,
...
mod tests {
    ...
    async fn telemetry_detects_duplicate_runs_and_post_integrating_runs() {
        ...
        let pool = spool_db::connect(&temp.path().join("spool.db"))
            .await
            .expect("connect");
...
"#,
        ...
        assert_eq!(
            ...
            spool_db::CURRENT_AGENT_RUN_METRICS_DERIVATION_VERSION
        );
        ...
        assert_eq!(
            spool_db::get_agent_run_metrics(&pool, "exact-run")
                .await
                .expect("metrics")
                .expect("old row")
                .input_tokens,
        ...
        let exact = spool_db::get_agent_run_metrics(&pool, "exact-run")
            .await
            .expect("metrics")
            .expect("updated row");
        assert_eq!(
        ...
        assert!(spool_db::get_agent_run_metrics(&pool, "proxy-run")
            .await
            .expect("metrics")
            .is_some());
    ...
    async fn telemetry_correlation_groups_before_and_after_landing_points() {
        ...
        sqlx::query(
            ...
            ) VALUES ('queue-1', 'TASK', 'Spool', '/repo', 'main', '/worktrees', 'spool/{task_identifier}')
            "#,
        )
        .execute(&pool)
        .await
    ...
    async fn telemetry_trend_compares_efficiency_windows_and_redacts_json() {
        ...
        assert!(landing
            .conclusions
            .iter()
            .any(|line| line.contains("repeated Spool context fetches: improved")));
        assert!(landing
            .cautions
            .iter()
    ...
    async fn telemetry_recent_efficiency_reports_windows_json_and_unknown_tokens() {
        ...
        for (run_id, tool_calls, reads, spool_fetches, tokens, max_context, transcript) in [
    ...
    async fn telemetry_workflow_metrics_flags_regressions_against_local_baseline() {
        ...
        for (run_id, tool_calls, reads, fetches, tokens, transcript, hints) in [
            ...
            (
                ...
                90_000,
                4_000_000,
                r#"["repeated Spool context fetches"]"#,
            ),
        ] {
            sqlx::query(
                r#"
        ...
        assert!(summary
            ...
            .recommendations
            .iter()
            .any(|line| line.contains("Spool context once")));
    ...
    async fn seed_lifecycle_task(
        ...
        sqlx::query(
            ...
            INSERT OR IGNORE INTO task_queues (
                id, key, name, managed_source_repository, main_branch, worktree_root, branch_template
            ) VALUES (?, 'TASK', 'Spool', '/repo', 'main', '/worktrees', 'spool/{task_identifier}')
            "#,
        )
        .bind(queue_id)
        .execute(pool)
