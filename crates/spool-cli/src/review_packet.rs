...
pub(crate) fn write_review_packet(
    ...
    bundle: &spool_db::TaskContextBundle,
) -> std::io::Result<()> {
...
pub(crate) fn render_review_packet(bundle: &spool_db::TaskContextBundle) -> Result<String> {
    let mut out = Vec::new();
...
fn write_agent_run_summary(
    mut writer: impl Write,
    runs: &[spool_db::TaskContextAgentRun],
) -> std::io::Result<()> {
    writeln!(writer, "\nRecent Agent Runs:")?;
    if runs.is_empty() {
        writeln!(writer, "(none)")?;
...
fn write_integration_summary(
    mut writer: impl Write,
    outcome: Option<&spool_db::TaskContextIntegrationOutcome>,
) -> std::io::Result<()> {
...
mod tests {
    ...
    fn review_packet_includes_human_review_context_and_local_diff_summary() {
        ...
        assert!(text.contains("1. [passed] cargo test -p spool-cli review_packet"));
    ...
    fn sample_bundle(fixture: &GitFixture) -> spool_db::TaskContextBundle {
        let task_id = "task-1".to_string();
        let now = "2026-05-11 00:00:00".to_string();
        spool_db::TaskContextBundle {
            task: spool_db::TaskDetail {
                ...
                acceptance_criteria: vec![spool_db::AcceptanceCriterion {
                    id: "ac-1".to_string(),
                ...
                validation_items: vec![spool_db::ValidationItem {
                    id: "vi-1".to_string(),
                    task_id: task_id.clone(),
                    position: 1,
                    description: "cargo test -p spool-cli review_packet".to_string(),
                    status: "passed".to_string(),
                    waiver_reason: None,
                }],
                tags: vec!["review-sessions".to_string()],
                workpad_note: Some(spool_db::WorkpadNote {
                    id: "note-1".to_string(),
                    task_id: task_id.clone(),
            ...
            queue: spool_db::TaskContextQueue {
                key: "TASK".to_string(),
            ...
            local_workflow: spool_db::TaskLocalWorkflowContext {
                local_worktree: Some(fixture.repo.display().to_string()),
                task_branch: Some("spool/TASK-1".to_string()),
                main_branch: "main".to_string(),
                managed_source_repository: fixture.repo.display().to_string(),
                worktree_root: fixture.repo.parent().unwrap().display().to_string(),
                branch_template: "spool/{task_identifier}".to_string(),
            ...
            advisory_hints: spool_db::TaskContextAdvisoryHints {
                note: "advisory only".to_string(),
                task_conflict_hints: Vec::new(),
                likely_files_or_paths: Vec::new(),
            },
            agent_runs: vec![spool_db::TaskContextAgentRun {
                id: "run-1".to_string(),
            ...
            latest_failure: None,
            latest_integration_outcome: Some(spool_db::TaskContextIntegrationOutcome {
