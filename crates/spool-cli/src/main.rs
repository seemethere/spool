...
use serde::Serialize;
use spool_config::{ensure_data_dir, PathOverrides, SpoolConfig, SpoolPaths};
...
#[command(version)]
struct Cli {
    /// Override the Spool config file path.
    #[arg(long, global = true, env = "SPOOL_CONFIG")]
    config: Option<PathBuf>,

    /// Override the Spool data directory.
    #[arg(long, global = true, env = "SPOOL_DATA_DIR")]
    data_dir: Option<PathBuf>,

    /// Override the Spool SQLite database path.
    #[arg(long, global = true, env = "SPOOL_DB_PATH")]
...
enum Command {
    /// Initialize Spool local config, data directory, and database.
    Init,
    /// Manage Spool database schema migrations.
    Db {
        #[command(subcommand)]
        command: DbCommand,
    },
    ...
    /// Show Spool queue and Task State counts.
    Status {
    ...
    Work {
        ...
        /// Spool API URL exposed to launched pi sessions.
        #[arg(long)]
        api_url: Option<String>,
        /// Pi executable path.
        #[arg(long, default_value = "pi")]
        pi_bin: String,
        /// Spool Pi Extension file to load into pi.
        #[arg(long)]
    ...
    /// Supervise spool work --once workers.
    #[command(
        after_long_help = "Supervisor progress logs are intentionally compact for unattended batches. Use `spool status` or `spool monitor` when you want human-readable Task titles and richer context."
    )]
    Supervise {
        /// Task Queue Key to supervise.
        #[arg(long)]
        queue: String,
        /// Maximum number of concurrent spool work --once processes.
        ...
        #[arg(long)]
        watch: bool,
        /// Agent Launcher passed to default spool work --once workers.
        #[arg(long, default_value = "pi")]
        launcher: String,
    ...
    Review {
        ...
        actor: String,
        /// Spool API URL exposed to the launched Review Agent.
        #[arg(long, hide = true)]
        api_url: Option<String>,
        /// Pi executable path.
    ...
    Delegate {
        ...
        actor: String,
        /// Spool API URL exposed to the launched Delegating Agent.
        #[arg(long, hide = true)]
        api_url: Option<String>,
        /// Pi executable path.
        #[arg(long, default_value = "pi", hide = true)]
    ...
    /// Start the Spool Service.
    Serve {
        /// Override the service bind address.
        #[arg(long)]
        bind: Option<SocketAddr>,
    },
    /// Show the Spool CLI version.
    Version,
}
...
enum MergeCommand {
    ...
    #[command(
        after_long_help = "Use this operator recovery command when an Integrating Task's latest Integration Outcome is a retryable operational_failure and the local operational issue has been fixed. It re-runs only the Delivery Adapter path; it does not claim work, launch a new Agent Run, or re-run Worker Agent code. Use `spool task retry` for failed/stuck agent work that should return to Ready, and use Rework for work_change_failure outcomes that require Task changes."
    )]
    Retry {
        /// Task Identifier.
        identifier: String,
    ...
    Done {
        /// Task Identifier.
        identifier: String,
        /// Confirm that the operator already performed the Local Merge outside Spool.
        #[arg(long)]
        manual: bool,
        /// Operator actor display name for audit attribution.
        #[arg(long, default_value = "local-operator")]
...
async fn main() -> Result<()> {
    ...
    print_active_spool_context_for_mutation(&cli.command, &paths, db_path_overridden)?;
    ...
    match cli.command {
        ...
        None => {
            println!("Spool CLI skeleton. Run `spool --help` for usage.");
            Ok(())
        }
    }
}
...
impl Cli {
    fn paths(&self) -> Result<SpoolPaths> {
        SpoolPaths::from_env(PathOverrides {
...
fn guard_project_config(cli: &Cli, paths: &SpoolPaths, db_path_overridden: bool) -> Result<()> {
...
fn guard_project_config_from(
    ...
    if command_is_unsafe_mutation(&cli.command) && !cli.has_intentional_config_override() {
        anyhow::bail!(
            "refusing mutating Spool command because project config {} is present but inactive; active config is {} and active database is {}. Re-run with --config .spool/config.toml, set SPOOL_CONFIG, use bin/spool-local, or pass an intentional --data-dir/--db-path override.",
    ...
    eprintln!(
        "warning: project config {} is present but inactive; active config is {} and active database is {}. Use --config .spool/config.toml or bin/spool-local to target this project.",
        project_config.display(),
        paths.config_path.display(),
        config.database.path.display()
    );
...
fn discover_project_config(start: &Path) -> Option<PathBuf> {
    for ancestor in start.ancestors() {
        let candidate = ancestor.join(".spool/config.toml");
...
fn active_spool_context(
    command: &Option<Command>,
    paths: &SpoolPaths,
    db_path_overridden: bool,
) -> Result<ActiveSpoolContext> {
    let mut config = SpoolConfig::load_or_default(paths)?;
    if db_path_overridden {
        config.database.path = paths.db_path.clone();
    }

    Ok(ActiveSpoolContext {
...
fn render_active_spool_context(context: &ActiveSpoolContext) -> String {
    let mut output = format!(
        "active Spool context:\n  config: {}\n  data: {}\n  database: {}",
...
fn print_active_spool_context_for_mutation(
    command: &Option<Command>,
    paths: &SpoolPaths,
    db_path_overridden: bool,
) -> Result<()> {
    ...
    let context = active_spool_context(command, paths, db_path_overridden)?;
    eprintln!("{}", render_active_spool_context(&context));
    Ok(())
}

async fn open_pool(paths: &SpoolPaths, db_path_overridden: bool) -> Result<sqlx::SqlitePool> {
    let mut config = SpoolConfig::load_or_default(paths)?;
    if db_path_overridden {
        config.database.path = paths.db_path.clone();
    }
